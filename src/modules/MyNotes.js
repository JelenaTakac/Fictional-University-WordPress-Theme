...
class MyNotes {
   ...
   findNearestParentLi(el) {
    ...
    async deleteNote(e) {
        ...
        try {
            const response = await axios.delete(`/wp-json/wp/v2/note/${thisNote.getAttribute("data-id")}`);
    ...
    async updateNote(e) {
        ...
        try {
            const response = await axios.post(`/wp-json/wp/v2/note/${thisNote.getAttribute("data-id")}`, ourUpdatedPost);
            this.makeNoteReadOnly(thisNote);
            console.log("congrats!");
            console.log(response);
